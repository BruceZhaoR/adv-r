...
#>  sqrt(x)   863 1,390 1723  1,660 1,840  9,410   100
#>    x^0.5 8,220 8,500 9019  8,720 8,890 39,300   100
```
...
#> Unit: nanoseconds
#>  expr    min     lq  mean median     uq       max neval
#>   fun    184    206   421    260    318    13,700   100
#>    S3  1,020  1,480 10880  1,710  1,960   888,000   100
#>    S4 13,700 16,600 33388 17,600 18,400 1,030,000   100
#>    RC  8,220  9,500 41497 10,300 11,400 3,090,000   100
```



The bare function takes about 300 ns. S3 method dispatch takes an additional 1,000 ns; S4 dispatch, 20,000 ns; and RC dispatch, 10,000 ns. S3 and S4 method dispatch are expensive because R must search for the right method every time the generic is called; it might have changed between this call and the last. R could do better by caching methods between calls, but caching is hard to do correctly and a notorious source of bugs.
...
#> Unit: nanoseconds
#>      expr min  lq mean median  uq       max neval
#>   f(1, 2) 367 392  719    399 435 2,270,000 10000
#>  f2(1, 2) 680 708  852    722 786    26,400 10000
#>  f3(1, 2) 722 751  897    767 829    35,500 10000
#>  f4(1, 2) 758 791 1162    809 869 2,210,000 10000
```
...
#> Unit: nanoseconds
#>  expr min  lq mean median  uq     max neval
#>  f0() 157 179  283    187 193 537,000 10000
#>  f1() 193 220  364    227 235 593,000 10000
#>  f2() 223 251  435    257 266 582,000 10000
#>  f3() 259 284  543    290 300 912,000 10000
#>  f4() 288 310  571    319 331 602,000 10000
#>  f5() 318 338  627    345 364 596,000 10000
```
...
#> Unit: nanoseconds
#>           expr    min     lq  mean median     uq     max neval
#>       [32, 11] 12,600 13,600 15193 14,100 14,700  54,600   100
#>      $carb[32]  5,950  6,960  8086  7,750  8,550  19,200   100
#>  [[c(11, 32)]]  5,130  6,190 11984  6,750  7,340 509,000   100
#>     [[11]][32]  4,950  5,800  6606  6,460  6,970  15,000   100
#>       .subset2    341    432   519    488    550   2,750   100
```
...
#> Unit: microseconds
#>             expr   min    lq mean median    uq   max neval
#>       squish_ife 23.90 29.10 61.4  34.10 38.70 2,640   100
#>         squish_p 13.70 15.90 41.5  17.50 19.40 1,770   100
#>  squish_in_place  3.13  3.97 35.4   4.55  5.34 3,040   100
```
...
#> Unit: microseconds
#>             expr  min   lq  mean median   uq     max neval
#>  squish_in_place 3.92 5.03  5.97   5.52 5.84    31.5   100
#>       squish_cpp 2.59 2.98 17.36   3.23 3.53 1,380.0   100
```
...
#> Unit: milliseconds
#>          expr   min    lq  mean median    uq    max neval
#>  cond_sum_cpp  4.77  4.82  4.86   4.83  4.85   6.61   100
#>    cond_sum_r 11.40 12.50 13.89  12.70 12.80 139.00   100
```
